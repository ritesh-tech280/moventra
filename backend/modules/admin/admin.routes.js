...
router.get("/drivers/:driverId/documents", validId("driverId"), async (req, res) => res.json({ success: true, documents: await require("../../models/DriverDocument").find({ driverId: req.params.driverId }).select("documentType verificationStatus createdAt expiryDate rejectionReason") }));
